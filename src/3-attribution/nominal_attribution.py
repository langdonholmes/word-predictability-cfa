"""AttnLRP attribution profiling for nominal tokens across proficiency.

Examines how context routing patterns differ for NOUN/PROPN tokens across
proficiency levels. Goes beyond distance-only analysis to examine *what kinds
of context* the model uses — other nominals, verbs, determiners, etc. — and
whether these patterns vary by proficiency.

Subcommands:
    compute  — Sample nominal tokens, run AttnLRP, save attributions
    analyze  — Compare attribution profiles by proficiency tertile
    show     — Inspect individual token attribution maps

Example workflow:
    python nominal_attribution.py compute --device cuda
    python nominal_attribution.py analyze
    python nominal_attribution.py show
...
Outputs (in data/lrp_pilot/):
    token_pos_cache.parquet   — POS/dep_rel lookup for all tokens (cached)
    nominal_attrs.parquet     — one row per (target, context) pair
    nominal_meta.parquet      — one row per target token
    nominal_params.json       — hyperparameters and runtime stats
    fig/nominal_attribution.pdf — multi-panel comparison figure
"""
...
def cmd_compute(args):
    """Sample nominal tokens and compute AttnLRP attributions."""
    import spacy
    import torch
    ...
    print(f"\nComputing AttnLRP for {n_total} nominal tokens...")
    ...
    attrs_df.to_parquet(OUTPUT_DIR / "nominal_attrs.parquet", index=False)
    print(f"Saved {len(attrs_df)} attr rows → {OUTPUT_DIR / 'nominal_attrs.parquet'}")

    meta_df = pd.DataFrame(meta_rows)
    meta_df.to_parquet(OUTPUT_DIR / "nominal_meta.parquet", index=False)
    print(f"Saved {len(meta_df)} meta rows → {OUTPUT_DIR / 'nominal_meta.parquet'}")

    params = {
        "method": "AttnLRP",
        "model": model_name,
        "purpose": "nominal_attribution",
    ...
    with open(OUTPUT_DIR / "nominal_params.json", "w") as f:
        json.dump(params, f, indent=2)
    print(f"Saved params → {OUTPUT_DIR / 'nominal_params.json'}")


# ── analyze ──────────────────────────────────────────────────────────────

def cmd_analyze(args):
    """Compare nominal attribution profiles by proficiency tertile."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    from scipy.stats import mannwhitneyu

    attrs_path = OUTPUT_DIR / "nominal_attrs.parquet"
    meta_path = OUTPUT_DIR / "nominal_meta.parquet"
    if not attrs_path.exists() or not meta_path.exists():
        print(f"ERROR: output files not found in {OUTPUT_DIR}")
        print("Run `python nominal_attribution.py compute` first.")
        sys.exit(1)
    ...
    # ── Statistical tests ─────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("NOMINAL ATTRIBUTION: Proficiency comparison (low vs high)")
    print("=" * 80)
    ...
    # ── Figure ────────────────────────────────────────────────────────
    output_path = Path(args.output) if args.output else FIG_DIR / "nominal_attribution.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    fig, axes = plt.subplots(2, 3, figsize=(16, 10))
    fig.suptitle("Nominal Token Attribution Profiles by Proficiency", fontsize=13)

    tertile_colors = {"low": "#e74c3c", "mid": "#f39c12", "high": "#2ecc71"}
    tertile_labels = ["low", "mid", "high"]

    # Panel 1: Context POS share by proficiency (grouped bars)
    ax = axes[0, 0]
    pos_vars = [
        ("ctx_noun_share", "NOUN"),
        ("ctx_verb_share", "VERB"),
    ...
    # Panels 4-5: POS × distance heatmaps by tertile (low, high)
    ...
    # Panel 6: Top dep_rels by attribution share (all + by tertile)
    ax = axes[1, 2]
    top_n_deps = 8
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="AttnLRP attribution profiling for nominal tokens",
    ...
    p_compute = sub.add_parser(
        "compute", help="Sample nominal tokens, run AttnLRP"
    )
    p_compute.add_argument(
        "--model", choices=["bert", "modernbert"], default="bert",
    ...
    p_analyze.add_argument(
        ...
        help="Output PDF path (default: fig/nominal_attribution.pdf)"
    )
